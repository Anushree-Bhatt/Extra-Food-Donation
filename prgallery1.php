				...
				if($fileerror == 0){
						...
						//$fileDest="images/".$imagefullname;
							...
							if(empty($title) || empty($des) ){
									...
									window.open("prgallery.php");
									</script>
									<?php	
									header("Location:prgallery.php");
							}
							else{
									$query="INSERT INTO POSTS (title,des,imgfullname,oid) VALUES (?,?,?,?);";
									$stmt=mysqli_stmt_init($connection);
									if(! mysqli_stmt_prepare($stmt,$query)){
										header("Location:prgallery.php?statement_not_prepared");	
									}
									else{
											...
											move_uploaded_file($tmp_name,$path );
											header("Location:prgallery.php?upload_successful");
									}
							} 
			  }
			  else{
